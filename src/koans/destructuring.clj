...
(meditations
  ...
  (= "Rich Hickey aka The Clojurer aka Go Time aka Macro Killah"
     (let [[first-name last-name & aliases]
       ...
       (str first-name " " last-name " aka " (reduce str (interpose " aka " aliases)))))
  ...
  (= "123 Test Lane, Testerville, TX"
     (let [{street-address :street-address, city :city, state :state} test-address]
            (reduce str (interpose ", " [street-address city state]))))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [[first-name last-name] {:keys [street-address city state]}]
        (str first-name " " (reduce str (interpose ", " [last-name street-address city state])))) 
